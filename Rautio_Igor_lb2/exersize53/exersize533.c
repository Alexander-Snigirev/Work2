/*5.3.3. Задайте разные приоритеты процессам: 
как это повлияло на очередность исполнения процессов, подтвердите экспериментально.*/
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main() {
    ...
    params[1].sched_priority = 99;  // Процесс 2
    params[2].sched_priority = 1;   // Процесс 3
    params[3].sched_priority = 50;  // Процесс 4
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (pids[i] == 0) {
            ...
            if (sched_setscheduler(0, SCHED_FIFO, &params[i]) == -1) {
                perror("Ошибка установки SCHED_FIFO");
                exit(1);
            }
